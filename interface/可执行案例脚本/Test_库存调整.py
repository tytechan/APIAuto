...
class CaigouContractsTest(unittest.TestCase):
    ...
    def test01_get_bill_info(self):
        ''' 登陆相应环境壳后，查询库存调整单信息，便于下一案例用于查询箱子信息 '''
        # **************************** 案例公共信息初始化 ****************************
        ...
        try:
            ...
            # **************************** 返回值部分 ****************************
            # 获取下一案例请求信息
            order = combineJson(self.result,"rst","data","items",0,"order")
            global_config.set_value("ORDER",order)

            type = combineJson(self.result,"rst","data","items",0,"type")
            global_config.set_value("TYPE",type)
    ...
    def test02_get_boxes_info(self):
        ''' 根据库存调整单查询拣配单及新旧箱子信息 '''
        # **************************** 案例公共信息初始化 ****************************
        self.caseName = (lambda: sys._getframe(1).f_code.co_name)()
        global_config.set_value("CASENAME",self.caseName)
        ...
        try:
            ...
            # **************************** 返回值部分 ****************************
            # 获取新旧箱子信息及拣配单信息
            jianpidanhao = combineJson(self.result,"rst","data","outer","doc","code")
            ...
            for i in range(len(oldBoxesInfo)):
                if oldBoxesInfo[i].get("physics"):
                    ...
                    if combineJson(oldBoxesInfo,i,"physics","package") == "纸箱"\
                            and oldBoxesInfo[i].get("code") not in oldBoxArray:
                        oldBoxArray.append(oldBoxesInfo[i]["code"])

                newBoxArray.append(oldBoxesInfo[i].get("tbox"))

            loadProcessValue("#旧箱号",realValue=str(oldBoxArray))
            global_config.set_value("#旧箱号", oldBoxArray)
            loadProcessValue("#新箱号",realValue=str(newBoxArray))
            global_config.set_value("#新箱号", newBoxArray)
    ...
    def test03_get_adjust_type(self):
        ...
        try:
            ...
            # **************************** 返回值部分 ****************************
            # 获取调整类型代码
            adjustType = self.result["rst"]["data"]["adjust_type"].get(self.dict["TYPE"])
    ...
    def test08_tray_up_to_positon(self):
        ''' 扫码进行托盘入仓位 '''
        # **************************** 案例公共信息初始化 ****************************
        self.caseName = (lambda: sys._getframe(1).f_code.co_name)()
        global_config.set_value("CASENAME",self.caseName)
